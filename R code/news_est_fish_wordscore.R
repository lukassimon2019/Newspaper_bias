...
load(paste0("weights/",topic,"_filtered_weights.RData")) #filtered_weights_df
load(paste0("dfms/",topic,"_twit_dfm.RData")) #twit_dfm
subset_twit_dfm <- twit_dfm[, colnames(twit_dfm) %in% filtered_weights_df$words]
news_dfm_subset<-news_dfm[, colnames(news_dfm) %in% filtered_weights_df$words]
...
match_indices <- match(wordscores_names, filtered_weights_df$words)
ordered_weights <- filtered_weights_df$twit_fish_weights[match_indices]
...
mod_feat_plot <- feature_plot +   theme(
  ...
  labs(title = "Feature plot: Wahlrechtsreform")
mod_feat_plot
...
modified_plot <- plot +   geom_point(size = 2.5) +   theme(
  ...
  labs(title = "Newspaper estimation: Wahlrechtsreform")

modified_plot


ggsave(
  filename = paste0("pics/", topic, "/", topic, "_news_wordscores_plot.png"),
  plot = modified_plot,
  dpi = 300,  # Adjust DPI if needed
...
#Create empty df
#est_df <- data.frame(matrix(ncol = length(names(scaled_est)), nrow = 0))
#colnames(est_df) <- names(scaled_est)
...
modified_plot <- ggplot(top_features, aes(x = reorder(feature, chi2), y = chi2, fill = weights)) +
  ...
  labs(x = "Feature", y = "Chi2 Value", title = "Cannabis-SZ keyness analysis") +
  theme_minimal() +
  theme(
...
#keyness analysis
tstat_key<-textstat_keyness(news_dfm_subset,"zeit")
tstat_key<-tstat_key[match(names(filtered_scores),tstat_key$feature),]
stat_scored<-cbind(tstat_key[,c(1,2)],filtered_scores)
#View(stat_scored)

# Sort and select top 15 features by chi2 value
top_features <- stat_scored[order(-stat_scored$chi2), ][1:15, ]
# Create a custom color scale
scale_color <- scale_fill_gradient2(
  low = "red", mid = "white", high = "green", 
  midpoint = 0, 
...
# Create the bar chart
ass_plot<-ggplot(top_features, aes(x = reorder(feature, chi2), y = chi2, fill = filtered_scores)) +
  geom_bar(stat = "identity") +
  scale_color +
  labs(x = "Feature", y = "Chi2 Value", title = "Top 10 Features by Chi2 Value") +
  theme_minimal() +
  coord_flip() +
  ggtitle("Zeit associations: Bürgergeld")
ass_plot
ggsave(paste0("pics/",topic,"/",topic,"_zeit_association_plot.png"),ass_plot)
