...
prepro_func <- function(input_df) {
  ...
  stopwords_names_path <- "stop_plen_names.txt"
  stopwords_names <- readLines(stopwords_names_path)
  ...
  tok_party_raw <- tokens_select(tok_party_raw, pattern = c("@*",stopwords,stopwords_names), selection = "remove", padding = FALSE)
  tok_party <- tokens_tolower(tokens(tok_party_raw, remove_punct = TRUE, remove_symbols = TRUE, remove_numbers = TRUE, split_hyphens = FALSE, remove_url=TRUE))
  
  #stemming
  tok_stem<-tokens_wordstem(tok_party, language = "german")
  
  # Remove stopwords again (in case lemmatization reintroduced any)
  tok_stem <- tokens_select(tok_stem, pattern = c(stopwords,stopwords_names), selection = "remove", padding = FALSE)
